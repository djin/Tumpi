...
public class SwipeViewPagerAdapter extends FragmentStatePagerAdapter {
	...
	public SwipeViewPagerAdapter(FragmentManager fm) {
		...
		if (manager.listasCanciones.isEmpty()) {
	...
	public void crearLista(final ViewGroup container) {
		...
		final View v = inflater.inflate(R.layout.style_alert_crear_lista,
				container, false);
		alert.setView(v);
		// alert.setView(input);
		...
		alert.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int whichButton) {
				EditText input = (EditText) v
						.findViewById(R.id.txtInputNombreLista);
				...
				if (!value.equals("")) {
					...
					((ListaFragment) listasCreadas.get(numeroListas)).datos = manager
							.getLista(numeroListas).getCanciones();
				} else {
					dialog.cancel();
				}
			}
	...
	public void borrarLista(ViewGroup container, int position) {
		if (hasLists()) {
			...
			((ListaFragment) listasCreadas.get(position))
