...
public class ListaCanciones extends ActionBarActivity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// Es para quitar los botones de compartir y favorito, para que no se
		// vean en la interfaz
		...
		if (cancion_sonando == null) {
			cancion_sonando = new Cancion("Nombre canción", "Artista", "Album",
					0, 0, false, false);
			text_playing.post(new Runnable() {
				public void run() {
					try {
						conex.conexion.enviarMensaje("0|"
	 ...
	 * inflater = getMenuInflater(); inflater.inflate(R.menu.menuactionbar,
	 * menu); MenuItem btn = menu.findItem(R.id.btnSettings);
	 * btn.setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
	 * public boolean onMenuItemClick(MenuItem item) { Intent intent = new
	 * Intent(ListaCanciones.this, Settings.class);
	 * 
	 * startActivity(intent); return true; } }); return true; }
	 */
	...
	public void interpretarLista(String[] canciones) {
		if (canciones[0].equals("empty")) {
			lista.add(new Cancion("El Dj no ha propuesto canciones",
					"En breves minutos aparecerá", " ", 0, 100, false, true));
		} else {
			ArrayList<Cancion> lista_aux = new ArrayList<Cancion>();
			for (String cancion : canciones) {
				String[] datos_cancion = cancion.split("\\*");
				if (!"1".equals(datos_cancion[5])) {
					lista.add(new Cancion(datos_cancion[1], datos_cancion[2],
							datos_cancion[3], Integer
									.parseInt(datos_cancion[0]), Long
									.parseLong(datos_cancion[4]), false, false));
				} else {
					lista_aux.add(new Cancion(datos_cancion[1],
							datos_cancion[2], datos_cancion[3], Integer
									.parseInt(datos_cancion[0]), Long
									.parseLong(datos_cancion[4]), false, true));
				}
			}
			if (lista_aux.size() > 0) {
	...
	public void onMessageReceive(final String men) {
		...
		text_playing.post(new Runnable() {
			public void run() {
				...
				if (!"exit".equals(message)) {
					...
					case 4:
						...
						cancion_sonando = new Cancion(datos_cancion[1],
								datos_cancion[2], datos_cancion[3], Integer
										.parseInt(datos_cancion[0]), Long
										.parseLong(datos_cancion[4]), false,
								"1".equals(datos_cancion[5]));
						refrescarCancionSonando();
						break;
					}
				} else {
					Toast.makeText(p, "Perdida la conexión con el servidor",
			...
			private boolean thereAreVotedSongs(String votedSongsString) {
				return votedSongsString != null
						&& votedSongsString.length() != 0;
			}
		});
	...
	public void sacarNotificacion() {
		BitmapDrawable bitmap = ((BitmapDrawable) ListaCanciones.this
				.getResources().getDrawable(R.drawable.logo_tumpi));

		NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(
				ListaCanciones.this).setSmallIcon(R.drawable.logo_tumpi)
		...
		Intent notIntent = new Intent(ListaCanciones.this,
				SeleccionAplicacion.class);

		PendingIntent contIntent = PendingIntent.getActivity(
				ListaCanciones.this, 0, notIntent, 0);
