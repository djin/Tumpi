...
public class ConnectionManager {
	...
	public Boolean logInBridge(String nick, final String uuid) throws Exception {
		AsyncTask<String, Void, Boolean> logThread = new AsyncTask<String, Void, Boolean>() {
			@Override
			protected Boolean doInBackground(String... nick) {
				return conexion.logBridge(nick[0], uuid);
			}
		};
		return logThread.execute(nick).get(5, TimeUnit.SECONDS);
	}
	...
	class conectar extends AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			try {
				...
				Log.e("Conexion",
						"Error al realizar la conexion con el bridge: " + ex);
				return false;
			}
		}
	}
